...
/*		final Button cb[]=new Button[1000];
		final Label negcod[]=new Label[1000];
		final Label negnpo[]=new Label[1000];
		final Label negzre[]=new Label[1000];
		final Label negdat[]=new Label[1000];
		final Label negsco[]=new Label[1000];
		final Label negven[]=new Label[1000];
		final Label negfis[]=new Label[1000];
		final Label negdif[]=new Label[1000];
		final Label negfisiva[]=new Label[1000];
		final Label negdifiva[]=new Label[1000];
		final Button nextday[]=new Button[1000];
		final Button fiscaliva[]=new Button[1000];
	*/	
		...
		//final DateTime datainizio= new DateTime(secondContent, SWT.CALENDAR_WEEKNUMBERS);
	//	final DateTime 
	//	datainizio= new DateTime(secondContent, SWT.CALENDAR);
	//	datainizio.setDate(year, month, day);
	//	datainizio.setDate(datainizio.getYear(), datainizio.getMonth(), datainizio.getDay()-1);
		...
		if (dbengine.equals("mssql")) 
			...
			try
				...
				//RECUPERO I NEGOZI
		//		ResultSet rsneg = s1.executeQuery("select distinct ven_neg, ven_npo, ven_dat, sum(ven_tot) as vendite,  when fis.fiscal is null then 0 else fis.fiscal end,  \r\n" + 
		...
		//				"				(select distinct fis_neg, fis_npo, fis_dat, sum(fis_day) as fiscal from fiscal_close where fis_dat=CAST(GETDATE()-1 AS Date) group by fis_neg, fis_npo, fis_dat)  as fis\r\n" + 
		//				"				on ven_neg=fis_neg and ven_npo=fis_npo and ven_dat=fis_dat   \r\n" + 
		//				"			    where ven_dat=CAST(GETDATE()-1 AS Date) and ven_sta='T' and ven_sgn='+' and ven_not not in ('FAT','DDT') group by ven_neg, ven_npo, ven_dat, fis.fiscal order by ven_neg, ven_npo\r\n" + 
		//				"				");
		///		ResultSet rsneg = s1.executeQuery("select distinct ven_neg, ven_npo, ven_dat, sum(CASE when ven_sgn='+' then ven_tot else (ven_tot*(-1)) end) as vendite, "
		//				+ "case when fis.fiscal is null then 0 else fis.fiscal end, case when fis.fiscal is null then (sum(CASE when ven_sgn='+' then ven_tot else (ven_tot*(-1)) end)-0) "
		//				+ "else (sum(CASE when ven_sgn='+' then ven_tot else (ven_tot*(-1)) end)-fis.fiscal) end as diff, ven_zre from vendite_stor full outer join (select distinct fis_neg, fis_npo, "
		//				+ "fis_dat, sum(fis_vnet) as fiscal from fiscal_close where fis_dat=CAST(GETDATE()-1 AS Date) group by fis_neg, fis_npo, fis_dat)  as fis	on ven_neg=fis_neg and ven_npo=fis_npo and ven_dat=fis_dat "
		//				+ "   where ven_dat=CAST(GETDATE()-1 AS Date) and ven_sta='T' and ven_not not in ('FAT','DDT') and ven_zre<>'0' group by ven_neg, ven_npo, ven_zre, ven_dat, fis.fiscal order by ven_neg, ven_npo");
				
				ResultSet rsneg = s1.executeQuery("select distinct  vendite_stor.ven_neg,  vendite_stor.ven_npo, CAST( vendite_stor.ven_dat as DATE), sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end) as vendite, case when fis.fiscal is null then 0 else fis.fiscal end, case when fis.fiscal is null then (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-0) else (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-fis.fiscal) end as diff, vendite_stor.ven_zre, case when iva.importo is null then 0 else iva.importo end, case when fis.fiscal is null then (case when iva.importo is null then 0 else iva.importo*(-1) end) else (case when iva.importo is null then fis.fiscal else fis.fiscal-iva.importo end) end, vensco.scodiff  from vendite_stor  full outer join (select distinct fis_neg, fis_npo, fis_dat, sum(fis_vnet) as fiscal from fiscal_close where fis_dat=CAST(GETDATE()-1 AS Date) group by fis_neg, fis_npo, fis_dat)  as fis	on  vendite_stor.ven_neg=fis_neg and  vendite_stor.ven_npo=fis_npo and  vendite_stor.ven_dat=fis_dat full outer join (select sum(fis_importo) as importo, fis_neg, fis_npo, fis_zre from fiscal_close_iva group by fis_neg, fis_npo, fis_zre) as iva on  vendite_stor.ven_neg=iva.fis_neg and  vendite_stor.ven_npo=iva.fis_npo and  vendite_stor.ven_zre=iva.fis_zre full outer join (select distinct ven_neg, ven_npo, max(ven_nsc)-count(distinct ven_nsc) as scodiff from vendite_stor where ven_sta='T' and ven_dat = CAST(GETDATE()-1 AS Date)  group by ven_neg, ven_npo) as vensco on vendite_stor.ven_neg=vensco.ven_neg and  vendite_stor.ven_npo=vensco.ven_npo where  vendite_stor.ven_dat=CAST(GETDATE()-1 AS Date) and  vendite_stor.ven_sta='T' and  vendite_stor.ven_not not in ('FAT','DDT') and  vendite_stor.ven_zre<>'0' group by  vendite_stor.ven_neg,  vendite_stor.ven_npo,  vendite_stor.ven_zre,  vendite_stor.ven_dat, fis.fiscal, iva.importo, vensco.scodiff order by  vendite_stor.ven_neg,  vendite_stor.ven_npo");
//				ResultSet rsneg = s1.executeQuery("select distinct  vendite_stor.ven_neg,  vendite_stor.ven_npo, CAST( vendite_stor.ven_dat as DATE), sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end) as vendite, case when fis.fiscal is null then 0 else fis.fiscal end, case when fis.fiscal is null then (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-0) else (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-fis.fiscal) end as diff, vendite_stor.ven_zre, case when iva.importo is null then 0 else iva.importo end, case when fis.fiscal is null then (case when iva.importo is null then 0 else iva.importo*(-1) end) else (case when iva.importo is null then fis.fiscal else fis.fiscal-iva.importo end) end, vensco.scodiff  from vendite_stor  full outer join (select distinct fis_neg, fis_npo, fis_dat, sum(fis_vnet) as fiscal from fiscal_close where fis_dat='"+datafis+"' group by fis_neg, fis_npo, fis_dat)  as fis	on  vendite_stor.ven_neg=fis_neg and  vendite_stor.ven_npo=fis_npo and  vendite_stor.ven_dat=fis_dat full outer join (select sum(fis_importo) as importo, fis_neg, fis_npo, fis_zre from fiscal_close_iva group by fis_neg, fis_npo, fis_zre) as iva on  vendite_stor.ven_neg=iva.fis_neg and  vendite_stor.ven_npo=iva.fis_npo and  vendite_stor.ven_zre=iva.fis_zre full outer join (select distinct ven_neg, ven_npo, max(ven_nsc)-count(distinct ven_nsc) as scodiff from vendite_stor where ven_sta='T' and ven_dat = '"+datafis+"'  group by ven_neg, ven_npo) as vensco on vendite_stor.ven_neg=vensco.ven_neg and  vendite_stor.ven_npo=vensco.ven_npo where  vendite_stor.ven_dat='"+datafis+"' and  vendite_stor.ven_sta='T' and  vendite_stor.ven_not not in ('FAT','DDT') and  vendite_stor.ven_zre<>'0' group by  vendite_stor.ven_neg,  vendite_stor.ven_npo,  vendite_stor.ven_zre,  vendite_stor.ven_dat, fis.fiscal, iva.importo, vensco.scodiff order by  vendite_stor.ven_neg,  vendite_stor.ven_npo");
				...
				while (rsneg.next())
					...
					negfisiva[i].setText(""+rsneg.getFloat(8));
					negdifiva[i]= new Label(firstContent, SWT.NONE);
					negdifiva[i].setText(""+rsneg.getFloat(9));
					negfisiva[i].setEnabled(false);
					nextday[i]=new Button(firstContent, SWT.CHECK);
					fiscaliva[i]=new Button(firstContent, SWT.CHECK);
					
					neg.add(rsneg.getString(1));
					result= "negozio: "+rsneg.getString(1)+" "+rsneg.getString(2)+" "+rsneg.getString(3)+" "+rsneg.getString(4)+" "+rsneg.getString(5)+" "+rsneg.getString(6)+" "+rsneg.getString(7)+" "+rsneg.getString(8)+" "+rsneg.getString(9)+" "+rsneg.getString(10);
			...
			catch (Exception e)
			{
				...
				dialog.setMessage("ERRORE: "+e+" Last record "+result);
				dialog.open();
				System.out.println("ERRORE: "+e+" Last record "+result);
			}
			...
			if (dbengine.equals("postgresql")) 
				...
				try
					...
					ResultSet rsneg = s1.executeQuery("select distinct ven_neg, ven_npo, ven_dat, sum(ven_tot) as vendite, case when fis.fiscal is null then 0 else fis.fiscal end, "
							+ "case when fis.fiscal is null then (sum(ven_tot)-0) else (sum(ven_tot)-fis.fiscal) end as diff from vendite_stor full outer join (select distinct fis_neg, "
							+ "fis_npo, fis_dat, sum(fis_day) as fiscal from fiscal_close where fis_dat=CAST(GETDATE()-1 AS Date) group by fis_neg, fis_npo, fis_dat)  as fis on "
							+ "ven_neg=fis_neg and ven_npo=fis_npo and ven_dat=fis_dat where ven_dat=CAST(GETDATE()-1 AS Date) and ven_sta='T' and ven_sgn='+' and ven_not not in "
							+ "('FAT','DDT') group by ven_neg, ven_npo, ven_dat, fis.fiscal order by ven_neg, ven_npo");
	...
	//
		...
		confermadata.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e1) {
				...
				Label sco=new Label(firstContent, SWT.NONE);
				sco.setText("DIFF NUM SCO");
		//		Label max=new Label(firstContent, SWT.NONE);
		//		dat.setText("MAX SCO");
				Label cont=new Label(firstContent, SWT.NONE);
				cont.setText("CONTABILE");
				Label fis=new Label(firstContent, SWT.NONE);
				...
				/*
				 if (dbengine.equals("mssql")) 
					...
					try
						...
						Statement s1 = con.createStatement();
						//RECUPERO I NEGOZI
				//		ResultSet rsneg = s1.executeQuery("select distinct ven_neg, ven_npo, ven_dat, sum(CASE when ven_sgn='+' then ven_tot else (ven_tot*(-1)) end) as vendite, case when fis.fiscal is null then 0 else fis.fiscal end,"
				//				+ " case when fis.fiscal is null then (sum(CASE when ven_sgn='+' then ven_tot else (ven_tot*(-1)) end)-0) else (sum(CASE when ven_sgn='+' then ven_tot else (ven_tot*(-1)) end)-fis.fiscal) end as diff, ven_zre, case when iva.importo is null then 0 else iva.importo end  "
				//				+ "from vendite_stor full outer join (select distinct fis_neg, fis_npo, fis_dat, sum(fis_vnet) as fiscal from fiscal_close where fis_dat='"+datafis+"' group by fis_neg, fis_npo, fis_dat)  as fis	on ven_neg=fis_neg and ven_npo=fis_npo and ven_dat=fis_dat "
				//				+ "full outer join (select sum(fis_importo) as importo, fis_neg, fis_npo, fis_zre from fiscal_close_iva group by fis_neg, fis_npo, fis_zre) as iva on ven_neg=iva.fis_neg and ven_npo=iva.fis_npo and ven_zre=iva.fis_zre "
				//				+ "where ven_dat='"+datafis+"' and ven_sta='T' and ven_not not in ('FAT','DDT') and ven_zre<>'0' group by ven_neg, ven_npo, ven_zre, ven_dat, fis.fiscal, iva.importo order by ven_neg, ven_npo");
								...
								if (dbengine.equals("mssql")) 
									...
									try
										...
										while (tot>=i)  
										{
											...
											{
												//RECUPERO I NEGOZI
												//COPIO EVENTUALI RECORD PRESENTI SU FISCAL CLOSE SU FISCAL CLOSE STOR SE A TABELLA NON ESISTE
											//	String rsmovnew = ("select * into fiscal_close_stor from fiscal_close where fis_dat='"+datafis+"' and fis_neg='"+negcod[i]+"' and fis_npo='"+negnpo[i]+"'");
											//	s1.executeUpdate(rsmovnew);
										...
										//		System.out.println("insert into fiscal_close_iva (fis_neg, fis_npo, '"+negzre[i].getText()+"', fis_ali, fis_importo, fis_imposta, fis_natura) \r\n" + 
								          //           " select ven_neg, ven_npo, ven_zre,  \r\n" + 
...
//				ResultSet rsneg = s1.executeQuery("select distinct  vendite_stor.ven_neg,  vendite_stor.ven_npo, CAST( vendite_stor.ven_dat as DATE), sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end) as vendite, case when fis.fiscal is null then 0 else fis.fiscal end, case when fis.fiscal is null then (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-0) else (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-fis.fiscal) end as diff, vendite_stor.ven_zre, case when iva.importo is null then 0 else iva.importo end, case when fis.fiscal is null then (case when iva.importo is null then 0 else iva.importo*(-1) end) else (case when iva.importo is null then fis.fiscal else fis.fiscal-iva.importo end) end, vensco.scodiff  from vendite_stor  full outer join (select distinct fis_neg, fis_npo, fis_dat, sum(fis_vnet) as fiscal from fiscal_close where fis_dat='"+datafis+"' group by fis_neg, fis_npo, fis_dat)  as fis	on  vendite_stor.ven_neg=fis_neg and  vendite_stor.ven_npo=fis_npo and  vendite_stor.ven_dat=fis_dat full outer join (select sum(fis_importo) as importo, fis_neg, fis_npo, fis_zre from fiscal_close_iva group by fis_neg, fis_npo, fis_zre) as iva on  vendite_stor.ven_neg=iva.fis_neg and  vendite_stor.ven_npo=iva.fis_npo and  vendite_stor.ven_zre=iva.fis_zre full outer join (select distinct ven_neg, ven_npo, max(ven_nsc)-count(distinct ven_nsc) as scodiff from vendite_stor where ven_sta='T' and ven_dat = '"+datafis+"'  group by ven_neg, ven_npo) as vensco on vendite_stor.ven_neg=vensco.ven_neg and  vendite_stor.ven_npo=vensco.ven_npo where  vendite_stor.ven_dat='"+datafis+"' and  vendite_stor.ven_sta='T' and  vendite_stor.ven_not not in ('FAT','DDT') and  vendite_stor.ven_zre<>'0' group by  vendite_stor.ven_neg,  vendite_stor.ven_npo,  vendite_stor.ven_zre,  vendite_stor.ven_dat, fis.fiscal, iva.importo, vensco.scodiff order by  vendite_stor.ven_neg,  vendite_stor.ven_npo");
			...
			catch (Exception e)
			{
				MessageBox dialog = new MessageBox(shell, SWT.ICON_WARNING | SWT.OK);
				dialog.setText("ATTENZIONE");
				dialog.setMessage("ERRORE3: "+e+" Last record "+result);
				dialog.open();
				System.out.println("ERRORE3: "+e+" Last record "+result);
			}
	 ...
	 static void caricadati() {
			...
			if (dbengine.equals("mssql")) 
				...
				try
					{
					//System.out.println("ok1 "+ datafis);
			...
			//				+ "where ven_dat='"+datafis+"' and ven_sta='T' and ven_not not in ('FAT','DDT') and ven_zre<>'0' group by ven_neg, ven_npo, ven_zre, ven_dat, fis.fiscal, iva.importo order by ven_neg, ven_npo");
					...
					ResultSet rsneg1 = s1.executeQuery("select distinct  vendite_stor.ven_neg,  vendite_stor.ven_npo, CAST( vendite_stor.ven_dat as DATE), sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end) as vendite, case when fis.fiscal is null then 0 else fis.fiscal end, case when fis.fiscal is null then (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-0) else (sum(CASE when  vendite_stor.ven_sgn='+' then  vendite_stor.ven_tot else ( vendite_stor.ven_tot*(-1)) end)-fis.fiscal) end as diff, vendite_stor.ven_zre, case when iva.importo is null then 0 else iva.importo end, case when fis.fiscal is null then (case when iva.importo is null then 0 else iva.importo*(-1) end) else (case when iva.importo is null then fis.fiscal else fis.fiscal-iva.importo end) end, vensco.scodiff  from vendite_stor  full outer join (select distinct fis_neg, fis_npo, fis_dat, sum(fis_vnet) as fiscal from fiscal_close where fis_dat='"+datafis+"' group by fis_neg, fis_npo, fis_dat)  as fis	on  vendite_stor.ven_neg=fis_neg and  vendite_stor.ven_npo=fis_npo and  vendite_stor.ven_dat=fis_dat full outer join (select sum(fis_importo) as importo, fis_neg, fis_npo, fis_zre from fiscal_close_iva group by fis_neg, fis_npo, fis_zre) as iva on  vendite_stor.ven_neg=iva.fis_neg and  vendite_stor.ven_npo=iva.fis_npo and  vendite_stor.ven_zre=iva.fis_zre full outer join (select distinct ven_neg, ven_npo, max(ven_nsc)-count(distinct ven_nsc) as scodiff from vendite_stor where ven_sta='T' and ven_dat = '"+datafis+"'  group by ven_neg, ven_npo) as vensco on vendite_stor.ven_neg=vensco.ven_neg and  vendite_stor.ven_npo=vensco.ven_npo where  vendite_stor.ven_dat='"+datafis+"' and  vendite_stor.ven_sta='T' and  vendite_stor.ven_not not in ('FAT','DDT') and  vendite_stor.ven_zre<>'0' group by  vendite_stor.ven_neg,  vendite_stor.ven_npo,  vendite_stor.ven_zre,  vendite_stor.ven_dat, fis.fiscal, iva.importo, vensco.scodiff order by  vendite_stor.ven_neg,  vendite_stor.ven_npo");
